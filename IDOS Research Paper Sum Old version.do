...
foreach indicator in `aggInd' {
egen mm`indicator'`year' = total(Def`indicator'`year'), missing
gen adef`indicator'`year' = abs(Def`indicator'`year')
egen amm`indicator'`year' = total(adef`indicator'`year'), missing
gen sdef`indicator'`year' = (Def`indicator'`year')^2
egen smm`indicator'`year' = total(sdef`indicator'`year'), missing
}
...
twoway (tsline `indicator',  cmissing(n)), ytitle(Absolute Mismatch) yscale(range(0 1.5)) ylabel(#5) ttitle(Year) title(`: var label `indicator'')
...
egen totAmm = rowtotal(amm*)
twoway (tsline totAmm), ytitle(Aggregate (Absolute) Mismatch) yscale(range(0 30)) ylabel(#7) title(Worldwide Aggregate (Absolute) Mismatch)
...
* Look at regular mismatch or squared mismatch if desired.
restore
...
*Dealing with few indicator observation years- correct needshare to be only among those for which there is AID data and not need data? But aid coverage is almost always better than need coverage...
...
*Analysis for countries by year across all indicators.
preserve
*Clear out variables
